...
const ServiceUsageForm = ({
  ...
  const serviceMap = useMemo(
    () => Object.fromEntries(services.map((s) => [s.service_id, s])),
    [services]
  );
    ...
    if (open) {
      if (initialValues) {
        ...
        const booking = bookings.find(
          (b) => b.booking_id === initialValues.booking_id
        );
  ...
  const renderBookingLabel = booking => {
    ...
    const roomNumbers =
      booking.bookingRooms?.map((br) => br.room?.room_number).filter(Boolean) ||
      [];
    const roomText =
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Select placeholder="Chọn booking" onChange={handleBookingChange}>
            {bookings.map((b) => (
        ...
        <Form.Item
          ...
          rules={[
            { required: true, message: "Vui lòng chọn phòng sử dụng dịch vụ" },
          ]}
          tooltip="Chọn chính xác phòng đang sử dụng dịch vụ"
        >
          <Select
            placeholder="Chọn phòng"
            disabled={
              !selectedBooking || selectedBooking.bookingRooms?.length === 0
            }
            allowClear
          >
          ...
          <Select placeholder="Chọn dịch vụ">
            {services.map((s) => (
              <Option key={s.service_id} value={s.service_id}>
                {s.name} - {parseFloat(s.price).toLocaleString("vi-VN")} VNĐ /{" "}
                {s.unit}
              </Option>
            ))}
          </Select>
